...
def createDownloading(filename, totalBits, currentBits, speed, time_val, tid=''):
    msg = (
        ...
        f'📊 Progreso: {porcent(currentBits, totalBits)}%\n\n'
        f'💾 Tamaño total: {sizeof_fmt(totalBits)}\n\n'
        f'📥 Descargado: {sizeof_fmt(currentBits)}\n\n'
        f'⚡ Velocidad: {sizeof_fmt(speed)}/s\n\n'
        f'⏱️ Tiempo estimado: {datetime.timedelta(seconds=int(time_val))}\n\n'
    )
    if tid != '':
        msg += f'/cancel_{tid}'
    return msg
...
def createUploading(filename, totalBits, currentBits, speed, time_val, originalname='', tid=''):
    ...
    msg = (
        ...
        f'⚡ Velocidad: {sizeof_fmt(speed)}/s\n\n'
        f'⏱️ Tiempo transcurrido: {datetime.timedelta(seconds=int(time_val))}\n\n'
    )
    if tid != '':
        msg += f'/cancel_{tid}'
    return msg
...
def createCompresing(filename, filesize, splitsize):
    ...
    return (
        ...
        f'📦 Tamaño de partes: {sizeof_fmt(splitsize)}\n\n'
        f'🔢 Cantidad de partes: {parts}\n\n'
    )
...
def createFinishUploading(filename, filesize, split_size, current, count, findex):
    return (
        ...
        f'📁 Nombre: {filename}\n\n'
        f'📊 Tamaño total: {sizeof_fmt(filesize)}\n\n'
        f'📦 Tamaño de partes: {sizeof_fmt(split_size)}\n\n'
...
def createFilesMsg(evfiles):
    msg = f'📁 Archivos guardados ({len(evfiles)})\n\n'
    i = 0
    for f in evfiles:
        try:
...
def createStat(username, userdata, isadmin):
    msg = (
        ...
        f'👤 Nombre: @{username}\n'
        f'👤 Usuario Moodle: {userdata["moodle_user"]}\n'
    ...
    msg += f'📏 Límite de zips: {sizeof_fmt(userdata["zips"] * 1024 * 1024)}\n\n'
    ...
    msg += f'🔐 Tokenización: {tokenize_str}\n\n'
    return msg
        f'📊 Progreso: {porcent(currentBits, totalBits)}%\n\n'
        f'💾 Tamaño total: {sizeof_fmt(totalBits)}\n\n'
        f'📥 Descargado: {sizeof_fmt(currentBits)}\n\n'
...
def createFileMsg(filename, files):
    if len(files) > 0:
        # Espacio único antes de los enlaces para separar limpiamente el bloque
        msg = '\n<b>🔗 Enlaces de descarga:</b>\n\n'
        for f in files:
            url = urllib.parse.unquote(f['directurl'], encoding='utf-8', errors='replace')
            msg += f"<a href='{url}'>➥ {f['name']}</a>\n"
