...
class MoodleClient(object):
    ...
    def getDirectUrl(self, url):
        ...
        direct = self.path + 'webservice/pluginfile.php/' + tokens[4] + '/user/private/' + tokens[-1] + '?token=' + self.userdata.get('token', '')
        return direct
    ...
    def parsejson(self, raw_json):
        """Parsea JSON de forma robusta con json.loads y respaldo manual"""
        try:
    ...
    def delteFile(self, name):
        """Elimina un archivo del gestor de archivos privados / borrador"""
        try:
            urlfiles = self.path + 'user/files.php'
            resp = self.session.get(urlfiles, proxies=self.proxy, timeout=10)
            soup = BeautifulSoup(resp.text, 'html.parser')
    ...
    # Alias para compatibilidad ortográfica
