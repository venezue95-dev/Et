...
class CallingUpload:
    ...
    def __call__(self, monitor):
        ...
        if self.time_total >= 1:
            clock_time = (monitor.len - monitor.bytes_read) / (self.speed or 1)
            if self.func:
...
class MoodleClient(object):
    ...
    def createBlog(self, name, itemid, desc="<p dir=\"ltr\" style=\"text-align: left;\">asd<br></p>"):
        try:
            ...
            entryid = ''
            if 'entryid=' in resp.url:
                entryid = resp.url.split('entryid=')[1].split('&')[0]
            elif 'id=' in resp.url:
                entryid = resp.url.split('id=')[1].split('&')[0]
            return {'name': name, 'desc': desc, 'id': entryid, 'url': resp.url, 'files': []}
        except:
            return {'name': name, 'desc': desc, 'id': '', 'url': '', 'files': []}

    def getBlogs(self):
        try:
            blogsurl = f'{self.path}blog/index.php?userid=' + self.userid
            resp = self.session.get(blogsurl, proxies=self.proxy)
            soup = BeautifulSoup(resp.text, 'html.parser')
            nodes = soup.find_all('div', {'class': 'blog_entry'})
            if not nodes:
                nodes = soup.find_all('div', {'class': 'forumpost'})
            
            blog_list = []
            for n in nodes:
                title_elem = n.find('h3', {'class': 'name'}) or n.find('a')
                blog_name = title_elem.text.strip() if title_elem else "Sin título"
                
                link_elem = n.find('a', href=re.compile(r'index\.php\?.*entryid='))
                blog_id = ''
                blog_url = ''
                if link_elem:
                    blog_url = link_elem['href']
                    if 'entryid=' in blog_url:
                        blog_id = blog_url.split('entryid=')[1].split('&')[0]
                
                file_links = n.find_all('a', href=re.compile(r'pluginfile\.php'))
                nfilelist = []
                for f in file_links:
                    url = str(f['href'])
                    directurl = url
                    try:
                        if 'token=' not in url and self.userdata and 'token' in self.userdata:
                            directurl = url + '&token=' + self.userdata['token']
                        directurl = str(directurl).replace('pluginfile.php', 'webservice/pluginfile.php')
                    except:
                        pass
                ...
                blog_list.append({'name': blog_name, 'desc': '', 'id': blog_id, 'url': blog_url, 'files': nfilelist})
            return blog_list
        except:
            return []
    ...
    def deleteBlog(self, blog):
        try:
            blogurl = f'{self.path}blog/edit.php?action=delete&entryid=' + str(blog['id']) + '&sesskey=' + self.sesskey
            resp = self.session.get(blogurl, proxies=self.proxy)
            soup = BeautifulSoup(resp.text, 'html.parser')
            form = soup.find('form')
            confirm_url = blogurl
            if form and form.get('action'):
                confirm_url = form['action']
            payload = {'sesskey': self.sesskey, 'confirm': 1, 'continue': 'Continuar'}
            self.session.post(confirm_url, data=payload, proxies=self.proxy)
            return True
        except:
            return False
    ...
    def upload_file_blog(self, file, blog=None, itemid=None, progressfunc=None, args=(), tokenize=False):
        try:
            ...
            if obj and obj.has_attr('data'):
                ...
                itempostid = query['itemid']
                ctx_id = query.get('ctx_id', '')
                env = query.get('env', '')
                maxbytes = query.get('maxbytes', '')
                areamaxbytes = query.get('areamaxbytes', '')
            else:
                itempostid = itemid or '0'
                ctx_id = ''
                env = 'filemanager'
                maxbytes = '0'
                areamaxbytes = '0'
