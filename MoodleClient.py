...
class MoodleClient(object):
    def __init__(self, user, passw, host='', repo_id=4, proxy=None):
        ...
        # Manejo robusto de proxy (soporta DirectProxyWrapper, ProxyCloud, dict o str)
        self.proxy = None
        if proxy:
            if hasattr(proxy, 'as_dict_proxy'):
    ...
    def createEvidence(self, name, desc=''):
        ...
        payload = {'userid': self.userid, 'sesskey': sesskey, '_qf__tool_lp_form_user_evidence': 1, 'name': name, 'description[text]': desc, 'description[format]': 1, 'url': '', 'files': files, 'submitbutton': 'Guardar+cambios'}
    ...
    def createBlog(self, name, itemid, desc="<p+dir=\"ltr\"+style=\"text-align:+left;\">asd<br></p>"):
        ...
        payload = {'action': 'add', 'entryid': '', 'modid': 0, 'courseid': 0, 'sesskey': self.sesskey, '_qf__blog_edit_form': 1, 'mform_isexpanded_id_general': 1, 'mform_isexpanded_id_tagshdr': 1, 'subject': name, 'summary_editor[text]': desc, 'summary_editor[format]': 1, 'summary_editor[itemid]': itemid, 'attachment_filemanager': attachment_filemanager, 'publishstate': 'site', 'tags': '_qf__force_multiselect_submission', 'submitbutton': 'Guardar+cambios'}
    ...
    def saveEvidence(self, evidence):
        ...
        payload = {'userid': self.userid, 'sesskey': sesskey, '_qf__tool_lp_form_user_evidence': 1, 'name': evidence['name'], 'description[text]': evidence['desc'], 'description[format]': 1, 'url': '', 'files': files, 'submitbutton': 'Guardar+cambios'}
        resp = self.session.post(saveevidence, data=payload, proxies=self.proxy, headers=self.baseheaders)
        return evidence
    ...
    def deleteEvidence(self, evidence):
        ...
        savejson = [{"index": 0, "methodname": "core_competency_delete_user_evidence", "args": {"id": evidence['id']}}, {"index": 1, "methodname": "tool_lp_data_for_user_evidence_list_page", "args": {"userid": self.userid}}]
    ...
    def upload_file_perfil(self, file, progressfunc=None, args=(), tokenize=False):
        ...
        resp3 = self.session.post(fileurl, data=payload, headers=self.baseheaders, proxies=self.proxy)
        return None, data

    def upload_file_draft(self, file, evidence=None, itemid=None, progressfunc=None, args=(), tokenize=False):
        try:
