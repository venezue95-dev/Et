...
def process_user_queue(username, bot, jdb):
    while True:
        with QUEUE_LOCK:
            ...
            if not q:
                if username in USER_WORKERS:
                    del USER_WORKERS[username]
                break
            task = q.pop(0)
            if not q:
...
def enqueue_user_task(username, task_data, bot, jdb):
    with QUEUE_LOCK:
        ...
        USER_QUEUES[username].append(task_data)
        position = len(USER_QUEUES[username])
        
        worker_running = username in USER_WORKERS and USER_WORKERS[username].is_alive()
        if not worker_running:
            t = threading.Thread(target=process_user_queue, args=(username, bot, jdb))
            t.daemon = True
            USER_WORKERS[username] = t
            t.start()
            return 1
    ...
    try:
        ...
        try:
            ...
            error_msg_user = (
                ...
                f"🌐 <b>Nube:</b> <code>{clean_host}</code>\n"
        ...
        if upload_result:
            ...
            if upload_type == 'evidence':
                ...
                try:
                    ...
                    moodle_client = MoodleClient(getUser['moodle_user'], getUser['moodle_password'], getUser['moodle_host'], getUser['moodle_repo_id'], proxy=proxy)
...
def onmessage(update,bot:ObigramClient):
    ...
    try:
        ...
        if MAINTENANCE_MODE and username.lower() != ADMIN_USERNAME.lower():
            bot.sendMessage(chat_id, "🛠️ <b>¡Sistema en mantenimiento temporal!</b>\n\n"
                                     "⚠️ <b>El bot se encuentra actualmente bajo labores de optimización y mantenimiento.</b>\n"
                                     "⏳ <b>Por favor, intenta de nuevo más tarde. Disculpa las molestias ocasionadas.</b>", parse_mode='html')
            return
        ...
        if username in CHANGING_CLOUD_USERS:
            if msgText.strip().isdigit():
                ...
                if 1 <= num <= len(AVAILABLE_CLOUDS):
                    ...
                    jdb.save()
                    CHANGING_CLOUD_USERS.discard(username)
                    bot.editMessageText(message, f"<b>✅ ¡Nube cambiada exitosamente!</b>\n\n🌐 <b>Nueva nube:</b> <code>{short_name}</code>\n⚖️ <b>Límite:</b> <b>{selected_cloud['zips']} MB</b>", parse_mode='html')
                    if LOG_GROUP_ID != 0 and username.lower() != ADMIN_USERNAME.lower():
                        try:
                            msg_log = (f"<b>🌐 ¡Cambio de nube!</b>\n\n"
                                       f"<b>👤 Usuario:</b> <b>@{username}</b>\n"
        ...
        if '/cambiar' in msgText:
            ...
            menu_msg = "🌐 <b>Selecciona tu nueva nube</b>\n\n"
...
🌐 <b>Nube actual:</b> <code>{admin_current_cloud}</code>
...
👤 <b>Usuario Regular</b>
👤 <b>Usuario:</b> <b>@{username}</b>
🌐 <b>Nube actual:</b> <code>{current_cloud_short}</code>
⚖️ <b>Límite:</b> <b>{user_info["zips"]} MB</b>
...
/cola - <b>Ver tus tareas y cola personal ⏱️</b>
/files - <b>Ver tus evidencias</b>
            ...
            try:
                if username.lower() == ADMIN_USERNAME.lower():
                    ...
                    for cfg in AVAILABLE_CLOUDS:
                        ...
                        if moodle_host in checked_hosts: continue
                    ...
                    for idx, cfg in enumerate(unique_configs):
                        ...
                        status_msg = f"🌐 <code>{clean_url}</code>\n<b>Estado:</b> <b>{icon}</b>"
                        if idx == 0:
...
"""
                ...
                if msgText == '/adm_userclouds':
                    try:
                        uclouds_msg = "🌐 <b>Gestión de nubes y usuarios</b>\n\n"
                        for idx, cloud_cfg in enumerate(AVAILABLE_CLOUDS, 1):
                            ...
                            assigned_users = []
                            for u in expanded_users.keys():
                                if u in REMOVED_USERS: continue
                ...
                elif msgText == '/adm_queues':
                    try:
                        with QUEUE_LOCK:
                            has_queues = bool(USER_QUEUES)
                        if not has_queues:
                            bot.editMessageText(message, "<b>✅ No hay colas activas en este momento.</b>", parse_mode='html')
                            return
                        
                        q_msg = "⏱ <b>Estado de las colas de usuarios</b>\n\n"
                        with QUEUE_LOCK:
                            for usr, q in USER_QUEUES.items():
                                size = len(q)
                                is_active = usr in USER_WORKERS and USER_WORKERS[usr].is_alive()
                                status_str = "🟢 Procesando" if is_active else "⚪ Inactivo"
                                q_msg += f"👤 <b>@{usr}:</b> <b>{size} tarea(s) en cola</b> ({status_str})\n"
                        send_long_message(bot, chat_id, q_msg, original_message=message, parse_mode='html')
                    except Exception as e:
...
"""
            ...
            active_task = None
            for tid, p in ACTIVE_PROCESSES.items():
                if p.get('user', '').lower() == username.lower():
                    active_task = p
                    break
            
            if not active_task and not user_queue:
                bot.editMessageText(message, "<b>✅ No tienes procesos activos ni elementos en cola.</b>", parse_mode='html')
                return
            
            cola_msg = "⏱ <b>Estado de tu cola personal</b>\n\n"
            
            if active_task:
                cola_msg += f"🟢 <b>Procesando ahora:</b>\n"
                cola_msg += f"• <b>{active_task.get('file', 'Archivo')}</b> ({active_task.get('action', 'Proceso')})\n"
                for tid, p in ACTIVE_PROCESSES.items():
                    if p.get('user', '').lower() == username.lower():
                        cola_msg += f"  ↳ /cancel_{tid}\n"
                cola_msg += "\n"
            
            if user_queue:
                cola_msg += f"⏳ <b>En espera ({len(user_queue)}):</b>\n"
                for idx, item in enumerate(user_queue, 1):
                    cola_msg += f"<b>{idx}.</b> <b>{item['filename']}</b> ({format_file_size(item['file_size'])})\n"
                    cola_msg += f"   ↳ /delq_{idx}\n\n"
                cola_msg += "<i>Usa /delq_X para eliminar un enlace de la cola.</i>"
            
            bot.editMessageText(message, cola_msg, parse_mode='html')
            return

        elif '/delq_' in msgText:
            try:
                idx = int(msgText.split('_')[1]) - 1
                with QUEUE_LOCK:
                    user_queue = USER_QUEUES.get(username, [])
                    if 0 <= idx < len(user_queue):
                        removed = user_queue.pop(idx)
                        try:
                            bot.deleteMessage(removed['message'].chat.id, removed['message'].message_id)
                        except:
                            pass
                        ...
                        bot.editMessageText(message, f"<b>🗑 Enlace eliminado de la cola:</b> <b>{removed['filename']}</b>", parse_mode='html')
                    else:
                        bot.editMessageText(message, "<b>❌ Índice inválido. Usa /cola para ver tu lista actual.</b>", parse_mode='html')
            except Exception as e:
                bot.editMessageText(message, f"<b>❌ Error al eliminar de la cola:</b> <b>{str(e)}</b>", parse_mode='html')
            return
            ...
            if username.lower() == ADMIN_USERNAME.lower():
                ...
                position = enqueue_user_task(username, task_data, bot, jdb)
                
                if position == 1:
                    send_reaction(chat_id, update.message.message_id, "⚡")
                    bot.editMessageText(message, f"<b>🚀 Tu proceso ha comenzado de inmediato.</b>", parse_mode='html')
                else:
                    send_reaction(chat_id, update.message.message_id, "⏳")
                    queue_msg = (
                        f"<b>⏱ Enlace añadido a tu cola personal</b>\n\n"
                        f"📁 <b>Archivo:</b> <b>{filename}</b>\n"
                        f"🔢 <b>Posición en cola:</b> <b>{position}</b>\n\n"
                        f"<i>Usa /cola para ver tus elementos en espera.</i>"
                    )
